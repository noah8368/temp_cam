...
int Mat2CImage(cv::Mat & src, CImage & dst)
{
	if (src.empty())
		return -1;
	int nBPP = src.channels() * 8;
	dst.Destroy();
	dst.Create(src.cols, src.rows, nBPP);
	if (nBPP == 8)
	{
		static RGBQUAD pRGB[256];
		for (int i = 0; i < 256; i++)
			pRGB[i].rgbBlue = pRGB[i].rgbGreen = pRGB[i].rgbRed = i;
		dst.SetColorTable(0, 256, pRGB);
	}
	uchar* psrc = src.data;
	uchar* pdst = (uchar*)dst.GetBits();
	int imgPitch = dst.GetPitch();
	for (int y = 0; y < src.rows; y++)
	{
		memcpy(pdst, psrc, src.cols*src.channels());//mat->step is incorrect for those images created by roi (sub-images!)
		psrc += src.step;
		pdst += imgPitch;
	}
	return 1;
...
void SetMedicalColorTable() {
	int temp[256][3] = { { 0, 0, 0 },{ 8, 0, 8 },{ 16, 0, 16 },{ 24, 0, 24 },{ 32, 0, 32 },{ 40, 0, 40 },{ 48, 0, 48 },{ 56, 0, 56 },{ 65, 0, 65 },
